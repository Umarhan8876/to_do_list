...
def con_db(fun):
    def wrap(*args, **kwargs):
        with psycopg2.connect("dbname=to_do_list2 user=postgres password = 2020 host=localhost port=5432") as conn:
            cur = conn.cursor()
...
def check(cur, user):
    id = input('tasks, id - ')
    cur.execute('update users set falled tasks = falled tasks+(select count())')